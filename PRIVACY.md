...
- 本地 RAG 模型按需从 Hugging Face 下载，包含固定版本模型与分词器，会消耗流量，服务方可见网络请求及 IP；模型下载请求不包含书籍、提示词或 API Key。下载后本地向量计算无需联网，启动或索引不会自动下载缺少的模型。
- WebDAV 仅在配置并启用后同步。书籍、笔记、阅读位置与时长按记录合并，删除保留标记；字体、主题图片、本机偏好和向量索引不参加书库同步。API Key 同步默认关闭，是独立开关；启用时要求独立加密密码并显示风险提示。
