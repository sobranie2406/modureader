...
- 「远程书库」是独立的 WebDAV 连接，仅浏览和下载，不修改服务器文件。地址和用户名保存在本机，不进入自动同步；密码仅当前运行期间保留。主动导出书库配置时，代码和二维码会包含地址、用户名，**默认也包含密码，可取消勾选**。这些代码和二维码未加密，不要公开分享；导入只填入设置表单，不自动连接或保存。Basic 认证不等于加密，推荐 HTTPS；明确允许 HTTP 会有凭据和传输内容被窃听的风险。
