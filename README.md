...
| AI 阅读技能 | 十个中文内置技能；启用/停用、查看和编辑提示词、新建自定义技能；对话保留技能标签，技能消息显示名称而非长提示词 | AI 对话输入区；设置 → AI 阅读技能 |
...
阅读样式 → 更多设置 → 样式提供 8 套可命名的 CSS 方案，可分别保存竖排、横排、精排等自定义代码。切换方案会保存当前编辑，启用后应用到正文；每本书在本机独立记住所选方案及启用状态，也可跟随默认方案或设为默认。修改同一套方案的代码会影响使用它的书籍。原有 CSS 保留到第 1 套，其余为空；方案本身不自动切换阅读器的横竖排模式。精排书可保留书籍样式并为该书关闭自定义 CSS。方案可随设置备份，按书选择仅在本机保存。
...
AI 对话中的快捷技能默认收起，可点输入栏旁的技能按钮展开。AI 正文字号可独立设置；思维导图支持导出 PNG、SVG、Markdown、FreeMind（`.mm`）和 JSON。
...
已下载模型可在模型设置中删除；正在使用的任务会限制删除。较大本地索引采用流式读取，单文件上限 1 GiB；实际可用规模仍受设备内存限制。
...
以下截图实拍于默读 **1.1.1（10033）macOS 正式版**。横排阅读使用项目原创示例书，古籍竖排使用本地《古文观止译注评（全二册）》；仅展示界面，不提供该书文件。截图中的主题、字体、模型与开关为演示时的设置，不代表默认值。
...
阅读页可展开 AI 侧栏，保留左侧正文。输入框上方的技能栏可横向滚动，提供本章总结、全书总结、概念解析、论证分析等已启用的技能；也可以输入自己的问题。截图展示操作入口，未发送请求或合成 AI 回复。
