...
| 远程书库 | 独立连接 WebDAV，浏览文件夹；按名称、添加/修改时间、大小升降序排列，搜索及格式筛选，下载导入书架 | 首页「远程书库」；设置 → 书库 WebDAV |
...
在「设置 → 书库 WebDAV」填写书籍目录的完整地址、用户名和密码，测试并保存后，从首页「远程书库」浏览。点击文件夹进入子目录，使用「上一级」或「根目录」返回；书籍右侧的下载按钮会将文件下载并导入本地书架，之后可离线阅读。支持进度显示、取消和重复书籍检查，单文件上限 512 MiB，一次下载一本；离开此标签会取消未完成的下载。

书库连接独立于 WebDAV 同步，只读取和下载，不上传或删除服务器文件。支持匿名访问和用户名/密码认证；推荐 HTTPS 和专用只读账号。地址、用户名和密码保存在本机应用配置中，重启后仍保留，本地不额外加密。开启「同步 API Key」后，书库配置及密码跟随 WebDAV 自动同步，以 AES-256-GCM 加密，各设备需使用相同同步加密密码。设置备份勾选「包含服务配置与 API Key（加密）」后也包含书库配置；普通未加密备份不包含。清除连接会删除保存的密码，并同步到其他已开启该开关的设备，不删除书籍或服务器文件。

可在书库 WebDAV 设置中主动导出 `modu:` 配置代码或二维码，也可导入代码或二维码图片。**默认导出密码，可手动关闭；代码和二维码未加密，不要公开分享。** 导入只填写设置表单，不会自动连接或保存。添加/修改时间来自服务器，缺失时显示未知，不会用本机导入时间冒充。

### 手机翻页方式

滚动模式下，点击翻页或使用翻页快捷键每次前进/后退阅读区域的 80%，保留 20% 重叠，便于连续阅读屏幕边缘的文字。自由滚动不受此步长限制。桌面正文支持方向键翻页；在 AI 输入框内，方向键仍用于移动光标。
...
以上四个模型**按需下载，不再内嵌在安装包中**。在「设置 → 向量模型」选择「下载并使用」，仅下载所选模型及分词器，通过文件大小与 SHA-256 校验后可离线计算，无需 API Key。下载来自 Hugging Face，会消耗网络流量；请保持下载页面打开，中断后可重试。旧版已准备的完整模型可继续复用。默认选择中文 BGE，自动向量化默认关闭；尚未下载时会提示先下载，不会自动消耗流量。也可选择远程嵌入 API。切换模型后应对旧书重新向量化；聊天和向量模型是两套配置。
...
选中文字或点击阅读页上方 AI 旁的翻译按钮即可翻译，支持 Google 翻译、AI 翻译和 DeepL/DeepLX。选文翻译统一正文字号并限制标题字号，长译文在与 AI 对话同规格的弹窗内滚动阅读。
...
- 书籍、笔记、书签和阅读位置逐条合并；阅读位置采用最近操作而非最远进度，新阅读时长按记录去重，删除保留标记。字体、主题图片和本地向量索引不随书库同步。并发上传使用服务器 ETag 校验；迁移与服务器要求见[同步说明](docs/WEBDAV_RECORD_SYNC.md)。
...
4. 需要语义检索时，先在「设置 → 向量模型」下载需要的本地模型（默认中文 BGE），再从书籍菜单建立索引；也可配置远程向量接口。
