...
| 配置迁移 | 全局设置、AI、朗读、同步与远程书库配置；文件、二维码和 modu 链接导入/导出 | 设置 → 高级 → 全局设置备份 |
...
阅读样式 → 更多设置 → 样式提供 8 套可命名的 CSS 方案，可分别保存竖排、横排、精排等自定义代码。切换方案会保存当前编辑，启用后应用到正文；每本书在本机独立记住所选方案及启用状态，也可跟随默认方案或设为默认。修改同一套方案的代码会影响使用它的书籍。原有 CSS 保留到第 1 套，其余为空；方案本身不自动切换阅读器的横竖排模式。精排书可保留书籍样式并为该书关闭自定义 CSS。支持多套独立开关和组合启用、文件导入导出，附 13 个可编辑模板及正则文字高亮规则；详见 [CSS 预设](docs/CSS_PRESETS.md)。方案可随设置备份，按书选择仅在本机保存。
...
请在“设置 → 高级 → 全局设置备份”选择远程书库配置，通过文件、`modu:` 链接或二维码图片迁移。**账号和密码默认不导出；选择包含后，文件、链接和二维码均为未加密数据，不要公开分享。** 导入前校验并确认，导入不自动连接服务器。添加/修改时间来自服务器，缺失时显示未知，不会用本机导入时间冒充。
...
朗读支持播放、暂停、恢复、上一句/下一句与章节切换。系统语音使用设备声音；在线语音可选择服务、音色和朗读参数。Xiaomi MiMo 提供官方音色下拉选择、可编辑的朗读风格与音色设计模板，以及可追加的常用描述提示词；支持 MP3/WAV，描述只作为声音指令，不混入正文。语音合成或播放失败时会暂停并保留当前位置，便于重试。
...
阅读页可展开 AI 侧栏，保留左侧正文。技能默认隐藏，点击星光按钮可在对话框内纵向展开本章总结、全书总结、概念解析、论证分析等已启用技能，选中后收起；也可以输入自己的问题。下图为此前界面的操作入口，当前技能列表已改为纵向弹出，未发送请求或合成 AI 回复。
