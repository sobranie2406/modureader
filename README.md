...
自由对话的正文搜索复用同一套关键词/向量混合检索；阅读技能继续限定当前章节、选文或阅读范围。设置 → 同步可独立开启索引同步，使用实际书籍文件 SHA-256 配对；接收端需先下载同一文件。索引含原文片段、不使用密钥同步加密，详情见[索引同步说明](docs/INDEX_SYNC_AND_READING_CONTROLS.md)。
...
- 书籍、笔记、书签和阅读位置逐条合并；阅读位置采用最近操作而非最远进度，新阅读时长按记录去重，删除保留标记。同步后拒绝旧页面进度回写，笔记冲突保留草稿。字体、主题图片和本地字典不随书库同步；向量索引可通过独立的「同步向量化数据」开关按需同步。可靠 ETag 服务使用条件写入，其他服务使用校验过的独立记录批次；迁移与服务器要求见[同步说明](docs/WEBDAV_RECORD_SYNC.md)。
