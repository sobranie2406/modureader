...
| 阅读与排版 | 目录跳转、相邻章节资源预加载、分页/滚动、字体字号与背景主题；注释自适应并保留末尾留白；TXT 按规则转为 EPUB | 阅读页面；设置 → 阅读 |
...
- 书籍、笔记、书签和阅读位置逐条合并；阅读位置采用最近操作而非最远进度，新阅读时长按记录去重，删除保留标记。同步后拒绝旧页面进度回写，笔记冲突保留草稿。字体、主题图片和本地向量索引不随书库同步。可靠 ETag 服务使用条件写入，其他服务使用校验过的独立记录批次；迁移与服务器要求见[同步说明](docs/WEBDAV_RECORD_SYNC.md)。
