...
**最新版本：1.1.6** — 在线语音按段连贯朗读、图形化 CSS 模板、内置选词搜索、清晰的笔记导出，以及可选的 GitHub / Gitee 更新来源。
...
| 配置迁移 | 一次迁移全局设置；文件、二维码和 modu 链接导入/导出，账号、密码及 API 接口配置独立开关 | 设置 → 高级 → 全局设置备份 |
...
复制、TXT 和 CSV 导出以 `原文：【…】` 标明摘录，多段原文放在同一对括号内，批注与原文分开。Markdown 在此基础上高亮备注，并显示笔记的创建或最后修改时间，方便整理和回顾。
...
远程书库配置随“设置 → 高级 → 全局设置备份”统一迁移，支持文件、`modu:` 链接和二维码图片。**账号、密码和 API 接口配置由独立开关控制，默认关闭；开启后导出的内容未加密，不要公开分享。** 关闭时导入也不覆盖本机凭据。导入前校验并确认，不自动连接服务器。添加/修改时间来自服务器。

设置文件或二维码保存成功后，会显示完整保存位置，可直接复制路径。Windows 的设置文件默认保存在当前用户的 Downloads（下载）文件夹；其他平台以保存对话框选择的位置为准。
...
MiMo、OpenAI 兼容、通义 DashScope 和 Edge 在线朗读将同一自然段的相邻句子合并合成，长段自动拆分，高亮和上一段/下一段与播放范围一致；系统语音保持逐句朗读。选中文字后点击「朗读」可从指定位置开始。首段优先生成，播放期间持续预加载后续段落，减少段落之间的等待；切换位置时及时取消旧结果的等待。

MiMo 提供官方音色、风格与音色设计模板、常用提示词，语速滑块直接调整本地播放倍率。OpenAI 兼容服务也提供描述预设、提示词编辑与开关。支持指令的接口自动附加稳定语速要求，保持所选音色与风格；描述不会混入正文。语音失败时保留位置供重试。
