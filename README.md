...
| 远程书库 | 独立连接 WebDAV 服务器，浏览文件夹、筛选当前目录，下载书籍并导入本地书架 | 首页「远程书库」；设置 → 书库 WebDAV |
...
书库连接独立于 WebDAV 同步，只读取和下载，不上传或删除服务器文件。支持匿名访问和用户名/密码认证；推荐 HTTPS 和专用只读账号。地址与用户名仅保存在本机，不参加设置导出或同步；密码只在当前运行期间保存，退出后需重新输入。
