...
## Modu 0.1.0-beta.3 Android hotfix (6330)

- Android 本地向量化改为原生端池化，仅返回最终向量，降低长书连续推理的数据复制与内存压力。
- 四个模型统一使用串行 CPU 推理，关闭内存池和形状缓存，及时释放输入、输出及空闲模型；实际输入限制为 512 tokens。
- 四个真实模型在 macOS Java 环境完成连续推理验证；Android 16 / iQOO Neo8 原书闪退仍待真机复测，不能保证覆盖系统强制回收或原生信号崩溃。
