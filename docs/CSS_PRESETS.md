...
导入支持普通 `.css` 或默读方案 `.json`，最多 1 MiB。JSON 保留名称、CSS、正则表达式、作用范围；可导出当前或全部非空方案。
...
- 尚未进行 Android/macOS 真机验证或生成新安装包。
