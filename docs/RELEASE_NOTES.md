...
- **设置输入与导出体验**：修复朗读接口设置中输入法重复上屏、在文字中间编辑时光标跳到末尾的问题。全局设置文件和二维码保存后显示完整保存位置，支持复制路径。
