...
新标签 `v0.1.0-beta.3`，构建号 `6328`；Beta 1、Beta 2 保留。Android 沿用原专用签名，可从相同签名的旧版覆盖升级。请先备份重要数据，不要为更新直接卸载旧版。
...
目标共 9 个应用包，以本发布页实际附件为准。Android `-notices.zip` 是许可附件，GitHub Source code 是源码，均非应用安装包。各附件附 SHA-256。Apple 营销版本为 `0.1.0`，构建号为 `6328`。

## 验证范围与已知限制

- 发布前本机：215 项 Flutter 测试通过、2 项联网测试跳过；19 项阅读器 JavaScript 测试通过。包含标题、空章、书末、并发导航、暂停恢复、停止取消和在线语音错误重试用例。
- 1 项 macOS 原生系统语音集成测试通过：三个测试句按顺序完成，其中包含两个标题。未使用私人书库，不等同于原问题书籍已验收。
- Android ARM64 Debug 构建通过，包内阅读器脚本与修复源码的生成结果一致。正式分发包由此标签的 CI 重新构建和校验。
