...
- **内置本地模型**：下载 ONNX 模型及分词器，选择当前模型，可进行本地推理测试。中文书可选择 BGE Small ZH v1.5，多语言书可选择 Multilingual E5 Small；另有两个英文模型。这里的名称是内置模型目录，不意味着模型文件已预装。
...
自动索引需要同时满足：启用向量模型、开启「导入后自动向量化」、本地模型已下载或远程接口配置有效。开关不会替你下载模型或补齐密钥。切换模型或修改源书内容后，应重新向量化。
