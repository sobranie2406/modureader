...
- **本地模型 · 离线内嵌**：安装包包含四个 ONNX 模型及分词器，显示「已内嵌 · 离线可用」。默认选择 BGE Small ZH v1.5；多语言书可选择 Multilingual E5 Small，另有两个英文模型。首次测试推理或向量化时，自动离线准备所选模型并校验大小与 SHA-256，无需下载或 API Key。已校验完整的旧模型继续复用；本地副本损坏时可从内嵌资源恢复。若安装资源缺失，可手动「下载并使用」从 Hugging Face 下载修复，会消耗网络流量，请保持页面打开直到完成。
