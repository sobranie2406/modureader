...
- **本地模型 · 按需下载**：安装包只含模型目录，不含四个 ONNX 权重及分词器。默认选择 BGE Small ZH v1.5；多语言书可选择 Multilingual E5 Small，另有两个英文模型。点击「下载并使用」从 Hugging Face 获取固定版本文件，下载完成后校验大小与 SHA-256，随后可测试推理并离线使用，无需 API Key。请保持页面打开，离开会中断下载，返回可重试；已校验完整的文件不会重复下载，旧版已准备好的模型可继续复用。模型和分词器都通过校验才显示「已下载」。
...
1.0.2 起使用数据库 8 的记录级合并，不再按整库文件时间替换本机数据库。阅读位置采用最近操作，新增时长去重，删除传播标记；字体不参加同步。所有客户端应一同升级，服务器需支持强 ETag 与条件写入；详见[同步与迁移说明](WEBDAV_RECORD_SYNC.md)。
