...
void FlutterOnnxruntimePlugin::RegisterWithRegistrar(flutter::PluginRegistrarWindows *registrar) {
  ...
  registrar->messenger()->SetMessageHandler("flutter_onnxruntime",
    ...
    if (size) job->request.assign(bytes, bytes + size);
    job->reply = std::move(reply);
    plugin_pointer->worker_->Post([plugin_pointer, job] {
      const auto& codec = flutter::StandardMethodCodec::GetInstance();
      try {
        auto call = codec.DecodeMethodCall(job->request.data(), job->request.size());
        if (!call) {
          job->response = codec.EncodeErrorEnvelope("INVALID_CALL", "Invalid inference request");
        } else {
          auto result = std::make_unique<flutter::MethodResultFunctions<flutter::EncodableValue>>(
              [job, &codec](const flutter::EncodableValue* value) {
                job->response = codec.EncodeSuccessEnvelope(value);
              },
              [job, &codec](const std::string& code, const std::string& message,
                            const flutter::EncodableValue* details) {
                job->response = codec.EncodeErrorEnvelope(code, message, details);
              },
              [] {});
          plugin_pointer->HandleMethodCall(*call, std::move(result));
        }
      } catch (...) {
        job->response = codec.EncodeErrorEnvelope("INFERENCE_ERROR", "Native inference request failed");
      }
      return [job] {
        if (job->response) job->reply(job->response->data(), job->response->size());
        else job->reply(nullptr, 0); // NotImplemented
        job->reply = nullptr;
      };
    });
  });
...
void FlutterOnnxruntimePlugin::HandleMethodCall(
  ...
  // Includes Ort::Env creation: first-use initialization belongs off the UI.
  if (!impl_) impl_ = std::make_unique<FlutterOnnxruntimePluginImpl>();
